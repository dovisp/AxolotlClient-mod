	...
	public void renderComponent(GuiGraphics graphics, float delta) {
		...
		if (minimal.get()) {
			...
			currPos = DrawUtil.drawString(graphics, df.format(x), currPos, pos.y() + 2, secondColor.get().toInt(),
				shadow.get());
			currPos = DrawUtil.drawString(graphics, separator, currPos, pos.y() + 2, separatorColor.get().toInt(), shadow.get());
			currPos = DrawUtil.drawString(graphics, df.format(y), currPos, pos.y() + 2, secondColor.get().toInt(),
				shadow.get());
			currPos = DrawUtil.drawString(graphics, separator, currPos, pos.y() + 2, separatorColor.get().toInt(), shadow.get());
			currPos = DrawUtil.drawString(graphics, df.format(z), currPos, pos.y() + 2, secondColor.get().toInt(),
				shadow.get());
