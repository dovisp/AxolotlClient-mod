...
public class CompassHud extends TextHudEntry implements DynamicallyPositionable {
	...
	public final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath("kronhud", "compasshud");
	...
	public void renderCompass(GuiGraphics graphics, float delta) {
		...
		float degrees = (client.player.getYRot(delta) + 180) % 360;
		if (degrees < -180) {
			degrees += 360;
		}
		...
		graphics.pose().translate(shift, 0);
		for (int i = 0; i < amount; i++) {
			...
			if (indicator == Indicator.CARDINAL) {
				...
				graphics.fill(innerX, y, innerX + 1, y + 9, majorIndicatorColor.get().withAlpha(
					(int) (targetOpacity * majorIndicatorColor.get().getAlpha())).toInt());
				...
				// We have to call .color() here so that transparency stays
				graphics.fill(innerX, y, innerX + 1, y + 5, minorIndicatorColor.get().withAlpha(
					(int) (targetOpacity * minorIndicatorColor.get().getAlpha())).toInt());
			}
		}
		graphics.pose().translate(-shift, 0);
