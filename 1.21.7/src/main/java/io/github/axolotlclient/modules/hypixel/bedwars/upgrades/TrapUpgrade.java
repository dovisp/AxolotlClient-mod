...
public class TrapUpgrade extends TeamUpgrade {
	...
	public void draw(GuiGraphics graphics, int x, int y, int width, int height) {
		if (traps.isEmpty()) {
			...
			graphics.blit(RenderPipelines.GUI_TEXTURED, ResourceLocation.withDefaultNamespace("textures/item/barrier.png"), x, y, 0, 0, 16, 16, 16, 16, color.toInt());
		} else {
	...
	public enum TrapType {
		ITS_A_TRAP((graphics, x, y, width, height, unused) ->
			graphics.blitSprite(RenderPipelines.GUI_TEXTURED, Gui.getMobEffectSprite(MobEffects.BLINDNESS), x, y, 0, width, height)),
		COUNTER_OFFENSIVE((graphics, x, y, width, height, unused) ->
			graphics.blitSprite(RenderPipelines.GUI_TEXTURED, Gui.getMobEffectSprite(MobEffects.SPEED), x, y, 0, width, height)),
		...
		MINER_FATIGUE((graphics, x, y, width, height, unused) ->
			graphics.blitSprite(RenderPipelines.GUI_TEXTURED, Gui.getMobEffectSprite(MobEffects.MINING_FATIGUE), x, y, 0, width, height));
