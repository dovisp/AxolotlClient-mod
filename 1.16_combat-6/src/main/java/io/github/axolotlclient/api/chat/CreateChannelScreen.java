...
public class CreateChannelScreen extends Screen {
	...
	protected CreateChannelScreen(Screen parent) {
		super(new TranslatableText("api.chat.groups.create"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		addButton(text("api.chat.groups.participants", leftColX, leftColY));
	...
	private <T> Consumer<Boolean> sliderAssembly(int x, int y, Function<Double, T> valueFunc, Consumer<Supplier<T>> value) {
		...
		var slider = new SliderWidget(x, y, 128, 20, LiteralText.EMPTY, 0) {
			@Override
