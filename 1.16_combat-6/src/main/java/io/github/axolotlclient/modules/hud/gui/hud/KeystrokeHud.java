...
public class KeystrokeHud extends TextHudEntry {
	...
	public static Optional<String> getMouseKeyBindName(KeyBinding keyBinding) {
		if (keyBinding.getBoundKeyTranslationKey().equalsIgnoreCase(
			InputUtil.Type.MOUSE.createFromCode(GLFW.GLFW_MOUSE_BUTTON_1).getTranslationKey())) {
			return Optional.of("LMB");
		} else if (keyBinding.getBoundKeyTranslationKey().equalsIgnoreCase(
			InputUtil.Type.MOUSE.createFromCode(GLFW.GLFW_MOUSE_BUTTON_2).getTranslationKey())) {
			return Optional.of("RMB");
		} else if (keyBinding.getBoundKeyTranslationKey().equalsIgnoreCase(
			InputUtil.Type.MOUSE.createFromCode(GLFW.GLFW_MOUSE_BUTTON_3).getTranslationKey())) {
	...
	public abstract class Keystroke {
		...
		public Map<String, Object> serialize() {
			...
			map.put("key", key.getBoundKeyTranslationKey());
	...
	private Keystroke deserializeKey(Map<String, Object> json) {
		if ("option".equals(json.get("type"))) {
			KeyBinding key = KeyBindAccessor.getAllKeyBinds().get((String) json.getOrDefault("key_name", json.get("option")));
	...
	public enum SpecialKeystroke {
		SPACE("space", new Rectangle(0, 54, 53, 7), MinecraftClient.getInstance().options.keyJump, (hud, stroke, matrices) -> {
		...
		LMB_CPS("lmb_cps", new Rectangle(0, 36, 26, 17), MinecraftClient.getInstance().options.keyAttack, (hud, stroke, graphics) -> {
			Rectangle bounds = stroke.bounds;
		...
		RMB_CPS("rmb_cps", new Rectangle(27, 36, 26, 17), MinecraftClient.getInstance().options.keyUse, (hud, stroke, graphics) -> {
			Rectangle bounds = stroke.bounds;
			int centerX = bounds.x() + stroke.offset.x() + bounds.width() / 2;
			int y = bounds.y() + stroke.offset.y() + 3;
			int nameY = y + bounds.height() / 4 - hud.client.textRenderer.fontHeight / 2;
		...
		public interface SpecialKeystrokeRenderer {
			void render(KeystrokeHud hud, Keystroke stroke, MatrixStack graphics);
		}
	}
}
